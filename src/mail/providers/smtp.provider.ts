  ...
  async sendMail(mailOptions: MailOptions): Promise<SentMessageInfo> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        this.logger.log(`SMTP mail gönderim denemesi ${attempt}/${maxRetries}: ${mailOptions.to}`);
        const result = await this.transporter.sendMail(mailConfig);
        
        this.logger.log(`SMTP mail başarıyla gönderildi: ${mailOptions.to}`);
        ...
        lastError = error;
        this.logger.log(`SMTP mail gönderim denemesi ${attempt}/${maxRetries} başarısız: ${error.message}`);
    ...
    this.logger.error(`SMTP mail gönderilemedi (${maxRetries} deneme sonrası): ${lastError.message}`);
    throw new Error(`SMTP mail gönderilemedi (${maxRetries} deneme sonrası): ${lastError.message}`);
  }
